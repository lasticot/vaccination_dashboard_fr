...
def make_sparkline(ax, df, metric):
    markers_on = [-1]
    ax.plot(df.index, df[metric], '-o', color=colors['sparkline'], markevery=markers_on)
    ax.fill_between(df.index, df[metric].min(), df[metric], color=colors['sparkline'], 
        alpha = 0.2)
    ax.axis('off')
    ax.margins(y=0.6)
    return ax
...
def make_card(ax, df, metric):
    ...
    value = f"{last * 100:.0f}" # .format(last * 1000) # human_format(last, k=True)
    pc = "({:+.2%})".format(pc)
...
def make_table(df, age=0):
    ...
    for idx, dep in enumerate(sorted.index):
        ...
        make_card(ax_card, df_dep, 'ratio')
